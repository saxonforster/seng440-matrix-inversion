...
@ matrix_optimized.c:626:     matrix_peak_magnitude_bits = MATRIX_INPUT_BITS;
	ldr	r2, .L76	@ tmp333,
@ matrix_optimized.c:575: {
	sub	sp, sp, #340	@,,
...
.LPIC3:
	add	r5, pc	@ ivtmp.156
@ matrix_optimized.c:627:     matrix_predicted_bits = MATRIX_INPUT_BITS;
...
@ matrix_optimized.c:636:     for (row = 0; row < N; row++) {
...
@ matrix_optimized.c:636:     for (row = 0; row < N; row++) {
	bne	.L9		@,
@ matrix_optimized.c:696:         if (peak_bits + growth_bits > matrix_predicted_bits) {
	ldr	fp, .L76+8	@ tmp437,
	movs	r1, #0	@ ivtmp.136,
...
.LPIC4:
	add	fp, pc	@ tmp437
@ matrix_optimized.c:644:     for (pivot_column = 0; pivot_column < N; pivot_column++) {
	mov	ip, r1	@ pivot_column, ivtmp.136
	str	r4, [sp, #16]	@ inverse, %sfp
@ matrix_optimized.c:625:     peak_bits = MATRIX_INPUT_BITS;
	movs	r0, #13	@ peak_bits,
	str	r0, [sp, #8]	@ peak_bits, %sfp
.L41:
@ matrix_optimized.c:657:         largest_value = fixed_absolute(augmented[pivot_column][pivot_column]);
	ldrsh	r0, [r10]	@ _7, MEM[(short int *)_438]
@ matrix_optimized.c:659:         for (row = pivot_column + 1; row < N; row++) {
...
@ matrix_optimized.c:656:         pivot_row     = pivot_column;
	mov	r7, ip	@ pivot_row, pivot_column
@ matrix_optimized.c:659:         for (row = pivot_column + 1; row < N; row++) {
	mov	r5, r9	@ row, row
.L12:
@ matrix_optimized.c:660:             current_value = fixed_absolute(augmented[row][pivot_column]);
...
@ matrix_optimized.c:664:                 pivot_row     = row;
...
@ matrix_optimized.c:659:         for (row = pivot_column + 1; row < N; row++) {
	...
	bne	.L12		@,
.L10:
@ matrix_optimized.c:672:         if (largest_value == 0) {
	cbz	r6, .L19	@ largest_value,
@ matrix_optimized.c:549:     return __builtin_clz(value);   /* one CLZ on Cortex-A7 */
	clz	r6, r6	@ _139, largest_value
@ matrix_optimized.c:696:         if (peak_bits + growth_bits > matrix_predicted_bits) {
	ldr	r5, [sp, #8]	@ _10, %sfp
@ matrix_optimized.c:690:         growth_bits = count_leading_zeros((uint32_t)largest_value) - 19;
...
@ matrix_optimized.c:697:             matrix_predicted_bits = peak_bits + growth_bits;
	it	gt
	strgt	r5, [fp, #4]	@ _10, matrix_predicted_bits
@ matrix_optimized.c:709:         if (pivot_row != pivot_column) {
	cmp	r7, r1	@ pivot_row, ivtmp.136
	beq	.L16		@,
	lsls	r7, r7, #5	@ _275, pivot_row,
@ /usr/lib/gcc/arm-linux-gnueabihf/14/include/arm_neon.h:10802:   return (int16x8_t)__builtin_neon_vld1v8hi ((const __builtin_neon_hi *) __a);
	mov	r0, lr	@ _15, ivtmp.144
@ matrix_optimized.c:711:             int16x8_t a_high = vld1q_s16(&augmented[pivot_row][N]);
	add	r4, r7, #16	@ _75, _275,
@ /usr/lib/gcc/arm-linux-gnueabihf/14/include/arm_neon.h:10802:   return (int16x8_t)__builtin_neon_vld1v8hi ((const __builtin_neon_hi *) __a);
	vld1.16	{d22-d23}, [r0:128]!	@ _85, MEM[(const short int[8] *)_14]
@ matrix_optimized.c:710:             int16x8_t a_low  = vld1q_s16(&augmented[pivot_row][0]);
...
@ matrix_optimized.c:733:         pivot_value = augmented[pivot_column][pivot_column];
	ldrsh	r0, [r10]	@ _7, MEM[(short int *)_438]
.L16:
@ matrix_optimized.c:735:         for (column = 0; column < AUGMENTED_N; column++) {
	movs	r4, #1	@ column,
@ matrix_optimized.c:735:         for (column = 0; column < AUGMENTED_N; column++) {
	add	r5, lr, #2	@ ivtmp.128, ivtmp.144,
@ matrix_optimized.c:737:             if (column == pivot_column) {
	cmp	r1, #0	@ ivtmp.136
	beq	.L18	@
...
@ matrix_optimized.c:271:     if (denominator == 0) {
	cbnz	r0, .L74	@ _7,
.L19:
@ matrix_optimized.c:673:             return MATRIX_SINGULAR;
	movs	r0, #0	@ <retval>,
@ matrix_optimized.c:891: }
	add	sp, sp, #340	@,,
	@ sp needed	@
	vldm	sp!, {d8-d11}	@,
...
.L20:
@ matrix_optimized.c:741:             if (!fixed_divide(augmented[pivot_column][column],
...
@ matrix_optimized.c:191:     if (value < INT16_MIN || value > INT16_MAX) {
	cmp	r6, #65536	@ _141,
	bcs	.L40		@,
@ matrix_optimized.c:750:             augmented[pivot_column][column] = (int16_t)division_result;
...
@ matrix_optimized.c:735:         for (column = 0; column < AUGMENTED_N; column++) {
	bne	.L75		@,
.L23:
@ matrix_optimized.c:785:             target_factor[target_count] = factor;
	ldr	r5, [sp, #4]	@ ivtmp.143, %sfp
@ /usr/lib/gcc/arm-linux-gnueabihf/14/include/arm_neon.h:10802:   return (int16x8_t)__builtin_neon_vld1v8hi ((const __builtin_neon_hi *) __a);
	mov	r0, lr	@ tmp365, ivtmp.144
@ matrix_optimized.c:754:         augmented[pivot_column][pivot_column] = FIXED_ONE;
	mov	r4, #4096	@ tmp364,
@ matrix_optimized.c:784:             target_row[target_count]    = row;
	add	r7, sp, #44	@ tmp431,,
@ matrix_optimized.c:754:         augmented[pivot_column][pivot_column] = FIXED_ONE;
...
@ matrix_optimized.c:777:         for (row = 0; row < N; row++) {
	movs	r0, #0	@ row,
@ matrix_optimized.c:775:         target_count = 0;
	mov	r4, r0	@ target_count, row
.L32:
@ matrix_optimized.c:778:             int16_t factor = augmented[row][pivot_column];
	lsls	r2, r0, #5	@ _413, row,
	ldrsh	r2, [r5, r2]	@ factor, MEM[(short int *)_411 + _413 * 1]
@ matrix_optimized.c:780:             if (row == pivot_column || factor == 0) {
	cmp	r2, #0	@ factor,
	it	ne		@
...
@ matrix_optimized.c:786:             target_count++;
	addne	r4, r4, #1	@ target_count, target_count,
@ matrix_optimized.c:777:         for (row = 0; row < N; row++) {
	cmp	r0, #8	@ row,
	bne	.L32		@,
@ matrix_optimized.c:791:         if (target_count > 0) {
	cmp	r4, #0	@ target_count
	beq	.L34	@
@ matrix_optimized.c:812:             stage_pointer = &augmented[target_row[0]][0];
	ldr	r2, [sp, #44]	@ target_row[0], target_row[0]
@ matrix_optimized.c:818:             for (index = 1; index < target_count; index++) {
	cmp	r4, #1	@ target_count,
@ matrix_optimized.c:815:             stage_factor  = target_factor[0];
...
@ matrix_optimized.c:789:         overflow = vdupq_n_s32(0);
...
@ matrix_optimized.c:821:                 next_pointer = &augmented[target_row[index]][0];
...
@ matrix_optimized.c:824:                 next_factor  = target_factor[index];
...
@ matrix_optimized.c:861:                 augmented[target_row[index]][pivot_column] = 0;
	movs	r6, #0	@ tmp421,
@ /usr/lib/gcc/arm-linux-gnueabihf/14/include/arm_neon.h:4638:   return (int32x4_t)__builtin_neon_vrshrs_nv4si (__a, __b);
...
@ matrix_optimized.c:860:             for (index = 0; index < target_count; index++) {
...
@ matrix_optimized.c:556:     return folded ? 32 - count_leading_zeros(folded) : 0;
	cmp	r4, #0	@ _151
	beq	.L34		@,
@ matrix_optimized.c:549:     return __builtin_clz(value);   /* one CLZ on Cortex-A7 */
	clz	r4, r4	@ _142, _151
@ matrix_optimized.c:871:             if (step_bits > peak_bits) {
...
@ matrix_optimized.c:871:             if (step_bits > peak_bits) {
	...
	bge	.L39		@,
@ matrix_optimized.c:873:                 matrix_peak_magnitude_bits = step_bits;
	ldr	r4, .L76+12	@ tmp424,
@ matrix_optimized.c:872:                 peak_bits                  = step_bits;
...
.L39:
@ matrix_optimized.c:876:             if (step_bits > MATRIX_MAGNITUDE_BITS) {
	cmp	r0, #15	@ iftmp.13_143,
	ble	.L34		@,
.L40:
@ matrix_optimized.c:747:                 return MATRIX_OVERFLOW;
	mov	r0, #-1	@ <retval>,
@ matrix_optimized.c:891: }
	add	sp, sp, #340	@,,
	@ sp needed	@
...
@ matrix_optimized.c:886:     for (row = 0; row < N; row++) {
...
@ matrix_optimized.c:886:     for (row = 0; row < N; row++) {
	...
	bne	.L42		@,
@ matrix_optimized.c:890:     return MATRIX_SUCCESS;
	movs	r0, #1	@ <retval>,
@ matrix_optimized.c:891: }
	add	sp, sp, #340	@,,
	@ sp needed	@
...
.L79:
@ matrix_optimized.c:925:                 sum += fixed_multiply(first_row[k], second[k][column]);
...
@ matrix_optimized.c:924:             for (k = 0; k < N; k++) {
...
@ matrix_optimized.c:924:             for (k = 0; k < N; k++) {
	bne	.L79		@,
@ matrix_optimized.c:928:             result[row][column] = sum;
	mov	r3, lr	@ tmp178, ivtmp.207
@ matrix_optimized.c:916:     for (row = 0; row < N; row++) {
...
matrix_infinity_norm:
	...
	@ frame_needed = 0, uses_anonymous_args = 0
	@ link register save eliminated.
@ matrix_optimized.c:945: {
	mov	r2, r0	@ tmp134, matrix
@ matrix_optimized.c:949:     int32_t maximum_row_sum = 0;
	movs	r0, #0	@ <retval>,
...
@ matrix_optimized.c:962:         if (row_sum > maximum_row_sum) {
	cmp	r0, r3	@ <retval>, _10
	it	lt
	movlt	r0, r3	@ <retval>, _10
@ matrix_optimized.c:951:     for (row = 0; row < N; row++) {
	cmp	r2, r1	@ ivtmp.222, _43
	bne	.L85		@,
@ matrix_optimized.c:968: }
	bx	lr	@
...
multiply_positive_q12:
	...
	@ frame_needed = 0, uses_anonymous_args = 0
	@ link register save eliminated.
@ matrix_optimized.c:984:     if (first < 0 || second < 0) {
	orrs	r3, r0, r1	@ tmp132, first, second
	bmi	.L91		@,
@ matrix_optimized.c:988:     if (first != 0 && second > INT32_MAX / first) {
	cbz	r0, .L89	@ first,
@ matrix_optimized.c:988:     if (first != 0 && second > INT32_MAX / first) {
...
@ matrix_optimized.c:988:     if (first != 0 && second > INT32_MAX / first) {
	...
	bgt	.L91		@,
.L89:
@ matrix_optimized.c:992:     product = first * second;
	mul	r3, r1, r0	@ product_13, second, first
@ matrix_optimized.c:997:     return 1;
	movs	r0, #1	@ <retval>,
@ matrix_optimized.c:995:     *result = product >> FRACTION_BITS;
...
@ matrix_optimized.c:997:     return 1;
	bx	lr	@
.L91:
@ matrix_optimized.c:985:         return 0;
	movs	r0, #0	@ <retval>,
@ matrix_optimized.c:998: }
	bx	lr	@
...
calculate_condition_number:
	...
	@ frame_needed = 0, uses_anonymous_args = 0
	push	{r4, r5, r6, lr}	@
@ matrix_optimized.c:1014: {
	mov	r5, r1	@ inverse, tmp131
	mov	r6, r2	@ condition_number, tmp132
@ matrix_optimized.c:1015:     int32_t matrix_norm  = matrix_infinity_norm(matrix);
	bl	matrix_infinity_norm(PLT)	@
	mov	r4, r0	@ tmp133,
@ matrix_optimized.c:1016:     int32_t inverse_norm = matrix_infinity_norm(inverse);
	mov	r0, r5	@, inverse
	bl	matrix_infinity_norm(PLT)	@
@ matrix_optimized.c:984:     if (first < 0 || second < 0) {
